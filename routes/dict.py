...
# 应用权限检查装饰器
@dict_bp.before_request
@check_access_blueprint('dict')
def check_access():
    pass  # 装饰器会处理权限检查逻辑
...
def dict():
    # 检查权限
    if not check_system_feature_access(session, 'dict.dict'):
    ...
    # 获取所有产品待办事项
    dicts = ProductBacklog.query.order_by(ProductBacklog.created_at.desc()).all()
...
@dict_bp.route('/dict/get/<int:backlog_id>')
def get_dict(backlog_id):
    try:
        # 检查权限
        if not check_system_feature_access(session, 'dict.dict'):
            return jsonify({'success': False, 'message': '权限不足'})
        ...
        print(f"Error in get_dict: {str(e)}")
        return jsonify({'success': False, 'message': f'获取需求信息失败: {str(e)}'}), 500


@dict_bp.route('/dict/edit/<int:backlog_id>', methods=['POST'])
def edit_dict(backlog_id):
    # 检查权限
    if not check_system_feature_access(session, 'dict.dict'):
        return jsonify({'success': False, 'message': '权限不足'})
...
@dict_bp.route('/dict/delete/<int:backlog_id>', methods=['POST'])
def delete_dict(backlog_id):
    # 检查权限
    if not check_system_feature_access(session, 'dict.dict'):
        return jsonify({'success': False, 'message': '权限不足'})
...
@dict_bp.route('/dict/import', methods=['POST'])
def import_dict():
    # 检查权限
    if not check_system_feature_access(session, 'dict.dict'):
        return jsonify({'success': False, 'message': '权限不足'})
